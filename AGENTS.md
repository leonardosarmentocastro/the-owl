...
- **`components/ui/` is the exception.** Files there are vendored, generated by
  the shadcn CLI, and keep its kebab-case registry names (`button.tsx`,
  `collapsible.tsx`) — do NOT rename them to PascalCase. The CLI maps a registry
  name to that exact path, so renaming breaks `shadcn add`/`diff` (duplicate
  re-adds, broken generated imports, lost update tracking). Our PascalCase rule
  applies only to hand-authored components.
